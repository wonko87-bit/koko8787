...
**설정 → 시스템 → "Windows 시작 시 자동 실행"** 을 켜면 `HKCU\...\Run` 에 지금 실행 중인 `.exe` 경로가 등록됩니다. 관리자 권한은 필요 없습니다.

**안 뜨는 경우는 보통 둘 중 하나이고, 둘 다 체크박스는 "켜짐"으로 보입니다.**

| 상태 | 왜 | 어떻게 |
|---|---|---|
| **예전 위치를 가리킴** | 등록에는 전체 경로가 들어갑니다. 새 빌드를 다른 폴더에 풀면 옛 경로는 없는 파일을 가리킵니다 | 앱을 한 번 실행하면 **자동으로 지금 위치로 고쳐집니다** |
| **Windows가 막아둠** | 작업 관리자 → 시작 프로그램에서 끄면 등록은 남고 무시만 합니다 | 체크를 **껐다 켜면** 풀립니다 |

설정 화면의 체크박스 아래에 지금 어느 상태인지 표시됩니다.
