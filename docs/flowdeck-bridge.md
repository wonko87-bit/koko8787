...
|---|---|---|
| 제목 템플릿 | `[읽기] {파일명}` | `Title` |
| 기한 | `수집일 + 7일` / `이번 주 금요일` / `없음` | `DueAt` |
| 시각 | `10:00` / `지정 안 함` | `DueAt`의 시간부, `HasTime` |
| 우선순위 | `보통` | `Priority` |
| 태그 | `리포트, 읽기` | `Tags` |
| 알림 | `30분 전` / `없음` | `ReminderMinutesBefore` |

파일명에서 날짜를 뽑는 기능(예: 계약서 파일명의 만기일)은 v1 범위 밖이다.
...
FileBox는 할일 하나를 만들 때 UUIDv4를 하이픈 없는 32자 소문자 hex로 만들어
**자기 store에 파일 엔트리와 함께 저장한다.** 같은 파일에 대해 다시 내보내는
상황(쓰기 실패 재시도, 사용자가 규칙을 다시 적용)에서는 저장해 둔 같은 `Id`를
재사용한다. 그래야 4.4의 중복 제거가 실제로 작동한다.
