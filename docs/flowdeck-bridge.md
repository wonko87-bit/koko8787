...
| 기한 | `없음` / `당일` / `1·3·7·14·30일 뒤` (수집일 기준) | `DueAt` |
...
규칙과 별개로 관리함의 파일 카드마다 **📋 Flowdeck** 버튼이 있다. 규칙을 만들 만큼
반복되지 않는 파일을 그때그때 보내기 위한 것으로, 걸리는 특별규칙이 있으면 그 설정을
쓰고 없으면 기한 없는 할일이 된다. 5.3 때문에 여러 번 눌러도 할일은 하나다.
